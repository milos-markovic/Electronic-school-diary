    ...
    @if(count($professors))
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    <td><img width='70' src="{{ asset('images/'.$professor->photo->name) }}" class="border" /></td>
                    <td><a href="{{ route( 'professor.details',$professor->id ) }}">{{ $professor->first_name }}</a></td>
                    <td>{{ $professor->last_name }}</td>
                    <td>{{ $professor->email }}</td>
                    <td>{{ $professor->role->name }}</td>
                    <td><a href="{{ route('professors.edit',$professor->id) }}" class="btn btn-outline-primary" >Update</a></td>
                    <td>
